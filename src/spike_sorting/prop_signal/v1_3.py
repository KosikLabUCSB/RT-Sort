"""
Combining all past versions' changes
"""
...
def merge_propagations_custom(propagations: list, elec_overlap_p: float, seq_order_p: float):
    """
...
def merge_propagations(propagations: list, min_elec_overlap_p: float, min_seq_order_p: float,
    ...
    clusters = fcluster(linkage_m, 1-min_seq_order_p/100, criterion="distance")
    propagations_merged = {}
...
def get_seq_order_score(prop_a, prop_b):
    ...
    seq_order_score = np.sum(matrices[0] * matrices[1]) / div

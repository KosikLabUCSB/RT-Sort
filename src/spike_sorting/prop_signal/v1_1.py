...
Changes since last version
    Renaming variables, parameters, and functions
    Added thresh_cooccurrences_p to rescan_candidate_cohorts
"""
...
def get_propagation_time(propagations, spike_times, prop_after):
    """
    ...
    for i in range(n):
        ...
        for n_ele in range(1, size_of_signal):  # ele = electrode
            ...
            ind_all = []
            for j in range(len(reference_spike_times)):
...
def rescan_candidate_cohorts(candidate_cohorts, thresh_cooccurrences_num, thresh_cooccurrences_p, p):
    """
    ...
    and more than thres_cooccurrences and more than thresh_cooccurrences_p*num_occurrences_on_first_electrode
    ...
    Inputs:
        ...
        thresh_cooccurrences_num: float
            Lower bound of the number of short latency co-occurrences each
            electrode needs to have.
        thresh_cooccurrences_p: float
            Each electrode needs to have thresh_cooccurrences_p * num_occurrences
    ...
    num_cohorts = len(candidate_cohorts)
    electrode_cohorts = [np.array([])] * num_cohorts
    ...
    for i in range(num_cohorts):
        if candidate_cohorts[i].size > 0:
            ...
            thresh_cooccurrences = max(thresh_cooccurrences_num, thresh_cooccurrences_p/100 * current_cohort[2, reference])
...
def scan_reference_electrode(spike_times, thresh_freq, seconds_recording, thresh_number_spikes, ratio,
    ...
    Inputs:
        ...
        seconds_recording: int or float
            ...
            2 minutes recording.
        thresh_number_spikes: int or None
            The threshold for total number of spikes on each reference
            electrode. If is None, use thresh_freq for threshold instead.
        ratio: float
    ...
    if thresh_number_spikes is not None:
        thresh = thresh_number_spikes
    else:
        thresh = thresh_freq * seconds_recording
    ...
    with Pool(initializer=_scan_reference_electrode_worker_init, initargs=(init_dict,), processes=20) as pool:
        for i, cohort_data in tqdm(enumerate(pool.imap(_scan_reference_electrode_func, range(n_e))), total=n_e):
...
def _scan_reference_electrode_func(electrode):
    ...
    thresh = _scan_reference_electrode_worker_dict["thresh"]
    ...
    # n = len(ref)  # Number of spikes detected by electrode
    #
    # if n >= 0 and n >= thresh:
    ...
    for electrode2 in range(n_e):
        ...
        if spikes_small_window >= ratio * spikes_big_window and spikes_big_window >= 1:
            time_delay = (delay - ccg_before) / sf
...
def prop_signal(spike_times, thresh_freq, seconds_recording, thresh_number_spikes, ratio, thresh_cooccurrences_num, thresh_cooccurrences_p, p,
                small_window=0.5, big_window=2, ccg_before=1.5, ccg_after=1.5, ccg_n_bins=61):
    """
    ...
    Inputs:
        ...
        seconds_recording: int, float, or None
            The length of recording in seconds. For example, 120 for
            2 minutes recording.
        thresh_number_spikes: int, float, or None
            The threshold for total number of spikes on each reference
            electrode. If is None, use thresh_freq for threshold instead.
        ratio: float
            Let n1 denote the largest sum of counts in any 0.5 ms moving
            ...
            of the first 2 ms window or the counts of the last 2 ms window
            as n2. This ratio is the lower bound threshold for n1/n2
        thresh_cooccurrences_num: int or float
            Lower bound of the number of short latency co-occurrences each
            electrode needs to have.
        thresh_cooccurrences_p: float
            Each electrode needs to have thresh_cooccurrences_p * num_occurrences
    ...
    electrode_cohorts = rescan_candidate_cohorts(candidate_cohorts, thresh_cooccurrences_num, thresh_cooccurrences_p, p)

...
def plot_spikes(spike_train, channel_idx, **plot_elec_probs_kwargs):
    ...
    plot_elec_probs(unit, **plot_elec_probs_kwargs)
...
def plot_elec_probs(seq, idx=None,
    ...
    if use_formation_spike_train:
        seq._spike_train = seq_spike_train
...
def form_coc_clusters(root_elec, time_frame, 
    ...
    for time in all_times:  # Two loops for this so tqdm is accurate
        ...
        traces = TRACES[max_amp_elecs, start_frame:rec_frame+n_after+1]
...
def setup_cluster(cluster,
    ...
    cluster.all_latencies = np.median(all_latencies, axis=0)# np.mean(all_latencies, axis=0)
    ...
    cluster.all_amp_medians = np.median(all_amp_medians, axis=0)# np.mean(all_amp_medians, axis=0)
...
def assign_spikes(all_units, time_frame, interelec=False,
    ...
    for unit in all_units:
        ...
        for root_elec in unit.root_elecs:
            ...
            comp_elecs, elec_probs, latencies, amp_medians = unit.root_to_stats[root_elec]
    ...
    for elec, time, amp in crossings:
        ...
        # if only_max_amps:  # 1/6/24 max-amp requirement is now unique to each sequence and is always required
        ...
        for unit in elec_watchers[elec]:
            ...
            latency_elec_weights, amp_elec_weights, \
            unit_latencies, unit_amp_medians = unit.assign_spikes_root_to_stats[elec]
...
def job(start_root_elec):
    # all_merges = []
    for root_elec in range(start_root_elec, min(start_root_elec+BATCH_SIZE, NUM_ELECS)):
        merges = form_from_root(root_elec, TRAINING_MS,
                    max_latency_diff=1.67, max_amp_median_diff=0.35)
        # utils.pickle_dump(merges, f"/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort/240109/merges_{root_elec}.pickle")
        for i, mer in enumerate(merges):
            utils.pickle_dump(list(mer.spike_train), f"/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort/240109/merges_{root_elec}_{i}.pickle")
...
if __name__ == "__main__":
    ...
    ALL_CROSSINGS = np.load("/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort/all_crossings.npy", allow_pickle=True)
    ELEC_CROSSINGS_IND = np.load('/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort/elec_crossings_ind.npy', allow_pickle=True)

    FRONT_BUFFER = 40  # Model's front sample buffer
    
    TRACES = np.memmap("/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/patch_rec_cat.si/traces_cached_seg0.raw", dtype="uint16", mode="r", shape=(29231400, 902)).T
    OUTPUTS = np.load("/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort/outputs.npy", mmap_mode="r")
...
LOOSE_THRESH = 0.157 # 0.1  # Decimal form (not percent)
LOOSE_THRESH_LOGIT = -1.6807211526535475 # -2.1972245773362196 # sigmoid_inverse(LOOSE_THRESH)  # For faster computation in spike splitting
INNER_RADIUS = 50
OUTER_RADIUS = 100
...
MIN_ELEC_PROB = 0.047 # 0.03  # If an elec's mean prob is less than MIN_ELEC_PROB, it is set to 0

CLIP_LATENCY_DIFF = 3.33# 5
CLIP_AMP_MEDIAN_DIFF = 1# 1000# 0.7

OVERLAP_TIME = 0.2  # For spike splitting and merging clusters with overlapping spikes
    
    # TRAINING_MS = (0, 5*60*1000)
    # from joblib import Parallel, delayed
    # from pathlib import Path

    # from math import ceil
    # BATCH_SIZE = ceil(NUM_ELECS / 20)
    # tasks = range(0, NUM_ELECS, BATCH_SIZE)
    # all_sequences = Parallel(n_jobs=20, verbose=231)(delayed(job)(t) for t in tasks)
    # utils.pickle_dump(all_sequences, "/data/MEAprojects/primary_mouse/patch_ground_truth/200724/2602/rt_sort/240109/all_sequences.pickle")

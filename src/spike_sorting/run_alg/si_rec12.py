...
def setup_cluster(cluster,
    ...
    for time in spike_train:
        ...
        latencies -= latencies[cluster.root_elec]
...
def assign_spikes(all_units, time_frame, interelec=False,
    ...
    for elec, time, amp in crossings:
        ...
        if spike_num_elecs > MAX_DETECTIONS:
            continue
...
def get_seq_spike_overlap(seq, time):
    ...
    unit_comp_elecs, unit_elec_probs, unit_latencies, unit_amp_medians, unit_root_amp_std = seq.root_to_stats[seq.root_elec]
...
if __name__ == "__main__":
    ...
    MAX_DETECTIONS = round(0.5*NUM_ELECS)

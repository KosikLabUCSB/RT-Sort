...
def calc_pre_mean_median(frame, elecs=slice(None), pre_frames=1000):
    """
...
def branch_coc_cluster(root_elec, comp_elecs,
    ...
    # assert False, "try using GMM instead of latency diff (same method as forming amp distribution)"
    ...
    # assert False, "allowed detections not detected by any other electrode need to be free to make cluster"
    ...
    # assert False, "maybe when picking next electrode, pick electrode closest to current comp_elec that has not already been examined"
...
def setup_elec_stats(cluster):
    ...
    # Find elecs
    cluster.loose_elecs = np.flatnonzero(cluster.all_elec_probs >= LOOSE_THRESH)
...
def assign_spikes(all_units, time_frame, interelec=False,
                  ...
                  overlap_time=0.1, overlap_dist=50, 
    ...
    for unit in all_units:
        ...
        for root_elec in unit.root_elecs:
            elec_to_idx = elec_to_seq_elecs_dict[root_elec]
            
            comp_elecs, elec_probs, latencies, amp_medians = unit.root_to_stats[root_elec]
            comp_elecs = [elec_to_idx[root_elec]] + [elec_to_idx[e] for e in comp_elecs if e != root_elec]  # root_elec is first elec
            
            loose_elecs = [elec_to_idx[e] for e in unit.loose_elecs]
            inner_loose_elecs = [elec_to_idx[e] for e in unit.inner_loose_elecs]
    ...
    for elec, time, amp in crossings:
        ...
        if only_max_amps: #  NOTE: If you stop requiring this before final sequences are formed, then you have to update Merge.merge to prevent merge pair having overlapping spikes (with this requirement, other than <<1% edge, case, this is not necessary) 
        ...
        if interelec:
            ...
            if len(spike_buffer) > 1 and time - spike_buffer[0][1] > overlap_time:
                split_interelec_spike(spike_buffer, time, overlap_time, overlap_dist, elec_to_outer_elecs)
        else:
...
def plot_seq_spike_overlap(seq, time, idx=0):    
    ...
    plt.suptitle(f"Inner: {num_inner_loose_elecs}/{MIN_INNER_LOOSE_DETECTIONS}. Loose: {num_loose_elecs}/{seq.min_loose_detections}. Latency: {latency_diff:.2f} frames. Amp: {amp_median_diff*100:.1f}%")
...
def _save_sequences_plots(args, n_spikes=5):
    ...
    for idx in np.random.choice(spike_train.size, n_spikes):
        ...
        plot_seq_spike_overlap(sequence, time, idx)
...
LOOSE_THRESH_LOGIT = -2.1972245773362196 # sigmoid_inverse(LOOSE_THRESH)  # For faster computation in spike splitting
INNER_RADIUS = 50
OUTER_RADIUS = 100

# from alg.automated_detection_propagation import automated_detection_propagation
# from plot import plot_propagations
from spikeinterface.extractors import NwbRecordingExtractor
import numpy as np
# from si import get_channel_map, save_inputs
...
def standardize_alg_outputs(list_of_propagation: list, propagating_times: list, lfp_nwb_path: str, max_electrodes=None):
    """
    Convert the units' pandas.DataFrames in list_of_propagation to lists in the standard format

    :param list_of_propagation:
        First output of automated_detection_propagation
    :param propagating_times:
        Second output of automated_detection_propagation
    :param lfp_nwb_path:
        Path to .nwb file containing LFP data that was used for algorithm inputs
    :param max_electrodes:
        Max number of electrodes to use for getting spike times
        I.e. if max_electrodes==3, then only first 3 electrodes in propagation cohort will be used for spike times (others will be ignored)
    ...
    channel_ids = NwbRecordingExtractor(lfp_nwb_path).get_channel_ids()

    if max_electrodes is None:
        max_electrodes = np.inf
    else:
        max_electrodes -= 1  # qth index in propagating_times[i] = spike times isolated with q+1 electrodes

    sequences = [
        channel_ids[prop["ID"].values]
        for prop in list_of_propagation
    ]  # Has no threshold for minimum latency while spike_time does
    spike_times = [
        times[min(len(times)-1, max_electrodes)]
        for times in propagating_times
    ]
    return sequences, spike_times


def threshold_crossings_npy_to_mat(thresh_crossings, mat_path):
    # Convert threshold crossings from npy array to .mat file
    mdict = {"crossing_times": thresh_crossings}
...
def run_with_allen():
    ...
    SPIKE_TIMES_PATH = f"/home/mea/SpikeSorting/prop_signal/allen_inst/cache_manifest/session_{SESSION_ID}/probe_{PROBE_ID}_thresh_crossings.npy"
    SPIKE_TIMES = np.load(SPIKE_TIMES_PATH, allow_pickle=True)
    THRES_FREQ = 1
    SECONDS_RECORDING = 9628.501091856735
    THRES_NUMBER_SPIKES = None
    RATIO = 0.5
    THRES_COOCCURRENCES = 50
    P = 50

    LIST_OF_PROPAGATION_PATH = f"/home/mea/SpikeSorting/prop_signal/allen_inst/cache_manifest/session_{SESSION_ID}/probe_{PROBE_ID}_list_of_propagation.npy"
    PROPAGATING_TIMES_PATH = f"/home/mea/SpikeSorting/prop_signal/allen_inst/cache_manifest/session_{SESSION_ID}/probe_{PROBE_ID}_propagating_times.npy"
    ...
    UNIT_SPIKE_TIMES_ALG_PATH = f"/home/mea/SpikeSorting/prop_signal/allen_inst/cache_manifest/session_{SESSION_ID}/probe_{PROBE_ID}_unit_spike_times_alg.npy"
    ...
    list_of_propagation, propagating_times = automated_detection_propagation(  # Run algorithm
        SPIKE_TIMES,
        THRES_FREQ,
        SECONDS_RECORDING,
        THRES_NUMBER_SPIKES,
    ...
    unit_sequences_alg, unit_spike_times_alg = standardize_alg_outputs(list_of_propagation, propagating_times, LFP_NWB_PATH)  # Standardize algorithm's outputs
    np.save(UNIT_SEQUENCES_ALG_PATH, unit_sequences_alg)  # Save unit sequences
    np.save(UNIT_SPIKE_TIMES_ALG_PATH, unit_spike_times_alg)  # Save unit spike times
    exit()

    unit_sequences_alg = np.load(UNIT_SEQUENCES_ALG_PATH, allow_pickle=True)  # Load alg unit sequences
    unit_spike_times_alg = np.load(UNIT_SPIKE_TIMES_ALG_PATH, allow_pickle=True)  # Load alg unit spike times
    print(len(unit_sequences_alg))  # Print number of units

    unit_sequences_allen = np.load(UNIT_SEQUENCES_ALLEN_PATH, allow_pickle=True)  # Load Allen unit sequences
    unit_spike_times_allen = np.load(UNIT_SPIKE_TIMES_ALLEN_PATH, allow_pickle=True)  # Load Allen unit spike times
    print(len(unit_sequences_allen))

    channel_map = np.load(CHANNEL_MAP_PATH, allow_pickle=True)[()]  # [()] is needed to unwrap dictionary that is nested in np.array
    print(len(channel_map))
    exit()
    # plot_unit_sequences(unit_sequences_alg, unit_sequences_allen, channel_map,)

    # Plot each propagation separately
    # for prop in list_of_propagation:
    #     plot_propagation_signal([prop], channel_map,
    #                             prop_color_a=0,
    #                             prop_color_b=0.1,
    #                             prop_alpha=0.5)
...
def run_with_si():
    NWB_PATH = "/data/MEAprojects/dandi/000034/sub-c1/sub-c1_ecephys.nwb"
    SPIKE_TIMES_PATH = f"/data/MEAprojects/dandi/000034/sub-c1/thresh_crossings_5.npy"
    LIST_OF_PROPAGATION_PATH = f"/data/MEAprojects/dandi/000034/sub-c1/propagations.npy"
    PROPAGATION_TIMES_PATH = f"/data/MEAprojects/dandi/000034/sub-c1/propagation_times.npy"

    SPIKE_TIMES = np.load(SPIKE_TIMES_PATH, allow_pickle=True)

    # list_of_propagation = np.load(LIST_OF_PROPAGATION_PATH, allow_pickle=True)
    # propagation_times = np.load(PROPAGATION_TIMES_PATH, allow_pickle=True)
    # unit_sequences, unit_spike_times = standardize_alg_outputs(list_of_propagation, propagation_times, NWB_PATH)
    # channel_map = get_channel_map(NWB_PATH)
    # for seq in unit_sequences:
    #     plot_unit_sequences([seq], [seq], channel_map)
    ...
    # idx = 1
    # ele2 = 3
    # propagation = list_of_propagation[idx].iloc[[0, -1]]  # [:ele2+1][0, -1]
    # propagation_times_ms = set(get_propagation_time_nonzero(list_of_propagation, SPIKE_TIMES, 1.5)[idx][-1])
    # propagation_times_ind = [i for i, st in enumerate(rec.get_times()) if st*1000 in propagation_times_ms]
    #
    # import matplotlib.pyplot as plt
    # channel_ids = rec.get_channel_ids()[propagation.ID.values]
    # st_buffer = 100
    # for st in propagation_times_ind:
    #     trace = rec.get_traces(start_frame=st-st_buffer, end_frame=st+st_buffer, channel_ids=channel_ids)
    #     for e in range(trace.shape[1]):
    #         plt.plot(trace[:, e], label=str(e))
    #     plt.axvline(st_buffer, color="black", linestyle="dashed")
    #     plt.legend()
    #     plt.show()


    THRES_FREQ = 1
    SECONDS_RECORDING = 270.011
    THRES_NUMBER_SPIKES = None
    RATIO = 0.5
    THRES_COOCCURRENCES = 50
    P = 100
    ...
    print(list_of_propagation)
    # unit_sequences_alg, unit_spike_times_alg = standardize_alg_outputs(list_of_propagation, propagating_times, LFP_NWB_PATH)  # Standardize algorithm's outputs
    np.save(LIST_OF_PROPAGATION_PATH, list_of_propagation)  # Save unit sequences
    np.save(PROPAGATION_TIMES_PATH, propagating_times)  # Save unit spike times


def run_with_maxone():
    REC_PATH = "/data/MEAprojects/organoid/intrinsic/200123/2953/network/data.raw.h5"
    THRESH_CROSSINGS_PATH = "/data/MEAprojects/organoid/intrinsic/200123/2953/network/thresh_crossings_maxwell.npy"

    THRESH_CROSSINGS_PATH = f"/data/MEAprojects/organoid/intrinsic/220318/14086/Network/000359/thresh_crossings_maxwell.npy"

    THRESH_CROSSINGS_PATH = f"/data/MEAprojects/dandi/000034/sub-mouse412804/prop_signal/thresh_crossings_5_neg_only.npy"

    thresh_crossings = np.load(THRESH_CROSSINGS_PATH, allow_pickle=True)
    spike_times = np.asarray([np.asarray(cs) for cs in thresh_crossings])

    print("Make sure to change seconds_recording")
    list_of_propagation, propagating_times = automated_detection_propagation(
        spike_times=spike_times,
        thres_freq=1,
        seconds_recording=900,
        thres_number_spikes=None,
        ratio=0.5,
        thres_cooccurrences=250,  # 50
        p=50,
...
def run_with_example():
    thresh_crossings = np.load("/home/mea/SpikeSorting/prop_signal/allen_inst/example/example_data.npy", allow_pickle=True)

    list_of_propagation_0_5 = np.load("/home/mea/SpikeSorting/prop_signal/allen_inst/example/list_of_propagation_0_5.npy", allow_pickle=True)

    list_of_propagation, propagating_times = automated_detection_propagation(
        spike_times=thresh_crossings,
        thres_freq=1,
    ...
    seq_len = [len(seq) for seq in list_of_propagation]
    print(len(seq_len))
    print(np.mean(seq_len))
    print(np.std(seq_len))
    print(np.median(seq_len))

    unique_elec = [len(set(seq.ID.values)) for seq in list_of_propagation]
    print(sum(unique_elec))


    # # Standardize list_of_propagation
    # width = 11
    # channel_map = {i: (i % width, (i // width)) for i in range(len(thresh_crossings))}
    # sequences = [
    #     prop["ID"].values
    #     for prop in list_of_propagation
    # ]  # Has no threshold for minimum latency while spike_time does
    ...
    #     plot_unit_sequences([seq], channel_map)


def run_with_dl():
    # for rec in ("200123_2957", "200520_5116", "200520_5118"):
    #     thresh_crossings = np.load(f"/data/MEAprojects/PropSignal/{rec}/thresh_crossings_5.npy", allow_pickle=True)
    #     print("DO YOU NEED TO CONVERT SAMPLES TO MS?")
    #     thresh_crossings = [np.array(x) for x in thresh_crossings]
    #     # thresh_crossings = [np.array(x) / 20 for x in thresh_crossings]  # /20 to convert from samples to ms
    #     list_of_propagation, propagating_times = automated_detection_propagation(
    #         spike_times=thresh_crossings,
    #         thres_freq=1,
    #         seconds_recording=180,
    ...
    #     )
    #     print(len(list_of_propagation))
    #     np.save(f"/data/MEAprojects/PropSignal/{rec}/default/list_of_propagation.npy", list_of_propagation)
    #     np.save(f"/data/MEAprojects/PropSignal/{rec}/default/propagating_times.npy", propagating_times)

    MODEL_PATH = "/data/MEAprojects/DLSpikeSorter/models/v0_4_4/2950/230101_133131_959516"

    thresh_crossings = np.load(f"{MODEL_PATH}/log/windows_200_120_spike_times.npy", allow_pickle=True)
    thresh_crossings = [np.array(x) / 20 for x in thresh_crossings]  # /20 to convert from samples to ms
    list_of_propagation, propagating_times = automated_detection_propagation(
        spike_times=thresh_crossings,
        thres_freq=1,
    ...
    print(len(list_of_propagation))
    np.save(f"{MODEL_PATH}/log/prop_signal/default/list_of_propagation.npy", list_of_propagation)
    np.save(f"{MODEL_PATH}/log/prop_signal/default/propagating_times.npy", propagating_times)


if __name__ == "__main__":
    run_with_maxone()
    # threshold_crossings_npy_to_mat(np.load("/data/MEAprojects/dandi/000034/sub-mouse412804/prop_signal/thresh_crossings_5_neg_only.npy", allow_pickle=True),
    #                                "/data/MEAprojects/dandi/000034/sub-mouse412804/prop_signal/thresh_crossings_5_neg_only.mat")

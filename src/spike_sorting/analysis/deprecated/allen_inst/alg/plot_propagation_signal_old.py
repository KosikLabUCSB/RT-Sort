...
def plot_propagation_signal(list_of_propagation, channel_map,
    ...
    Inputs:
        ...
        channel_map: list
            ...
            (x, y). I.e. the ith electrode in spike_times corresponds
            to the ith data in channel_map.
        axis: Axes
            Axis to plot on
            If None, a new figure will be created and automatically shown
        allen_units: None or pandas.DataFrame
            Result of sessions.get_units()
    ...
    if axis is None:
        # fig, axis = plt.subplots(1, figsize=(4, 15))  # type: Axes
        fig, axis = plt.subplots(1)  # type: Axes

        show = True
    else:
        show = False

    # Restructure channel and unit locations from 1d to 2d
    HEIGHT = 1000
    WIDTH = 100
    for i in range(len(channel_map)):
        ...
        channel_map[i] = (x, y)
    units = []
    for i in range(len(allen_units)):
        unit = allen_units.iloc[i]
        x, y = unit.probe_horizontal_position, unit.probe_vertical_position
        x += (y // HEIGHT) * WIDTH
        y %= HEIGHT
        units.append((x, y))
    ...
    # Plot kilosort's units from spikesort_matlab4.py
    ...
    xlim = axis.get_xlim()
    axis.set_xlim(xlim[0]-100, xlim[1]+100)
    if show:
        plt.show()

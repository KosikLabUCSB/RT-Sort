...
OUT_FILE = "spikesort_matlab4.out"
...
RESULTS_FOLDERS = INTERMEDIATE_FOLDERS[:]

# If COMPILE_RESULTS is True, extracted data from all recordings in RECORDING_FILES will be compiled together
COMPILED_RESULTS_FOLDER = "/home/maxlim/SpikeSorting/data/DL/sorted/6_compiled"

######################################################
###############  KILOSORT PARAMETERS  ################
######################################################
# Set Kilosort2's path
KILOSORT_PATH = "/home/mea/SpikeSorting/kilosort/Kilosort2"  # "/home/maxlim/SpikeSorting/Kilosort2"
# Set path of plugin (the path to the folder containing libcompression.so)
HDF5_PLUGIN_PATH = '/home/mea/SpikeSorting/spikeinterface'  # '/home/maxlim/SpikeSorting/extra_libraries/'
...
# If True, concatenated spike times across all units (spike_times) will be saved in .mat and .npy files
SAVE_SPIKE_TIMES = False
# If True, saves additional data in "unit_dict" of .mat and .npz files that are needed for DL spikesorter
SAVE_DL_DATA = False
...
RESULTS_FOLDER
        ...
        spike_times.npy     # The spikes that passed first curation - 10/4/24 NOT INCLUDED ANYMORE
        spike_clusters.npy  # Unit ids corresponding to spike_times.npy - 10/4/24 NOT INCLUDED ANYMORE
        parameters.json
...
%     end
    ...
    def format_params():
        if KILOSORT_PARAMS['NT'] is None:
            KILOSORT_PARAMS['NT'] = 16 * 1024 + KILOSORT_PARAMS['ntbuff']  # https://github.com/MouseLand/Kilosort/issues/380
        else:
...
def load_single_recording(rec_path):
    if str(rec_path).endswith(".raw.h5"):
        ...
        assert False, f"'{rec_path}' is not of .raw.h5 or .nwb"
...
def process_recording(rec_name, rec_path, inter_path, results_path):
    ...
    with Tee(Path(inter_path) / OUT_FILE, "a"):
        ...
        # Load Recording
        try:
            recording_filtered = load_recording(rec_path)
        except Exception as e:

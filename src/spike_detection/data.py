...
class Recording:
    ...
    def __init__(self, rec_path, sample_size, start, 
        ...
        # if str(traces_path).endswith(".h5"):  # Convert from .h5 to .npy for faster data retrieval
        #     raise NotImplementedError("Reading from .h5 is not implemented because recording contains 8 channels with basically nan values")
        #     self.traces = h5py.File(traces_path)['sig']
        #     # rec_path_path = Path(rec_path)
        #     # npy_path = str(rec_path_path.parent / rec_path_path.stem) + ".npy"
        #     # if not Path(npy_path).exists():
        #     #     Recording.write_npy(rec_path, npy_path)
        #     #
        #     # self.traces = np.load(npy_path, mmap_mode="r")
        # elif str(traces_path).endswith(".npy"):
        #     self.traces = np.load(traces_path, mmap_mode=mmap_mode)  # .astype("float32")
        #     # self.traces = torch.tensor(self.traces, dtype=torch.float32)
        #     # Casting to float32 may take too much RAM --> "exit code 137"
        # else:
        #     raise ValueError("traces_path does not lead to a .npy recording file")

        self.traces = np.load(rec_path / "traces.npy", mmap_mode=mmap_mode)
    ...
    def get_sample(self, channel=None):
        ...
        # sample = de.load_data(de.load_metadata(self.UUID), self.exp_num, sample_time, self.sample_size, channel)
        return self[channel, sample_time:sample_time+self.sample_size] * self.gain_to_uv
    ...
    @staticmethod
    def write_npy(rec_path, npy_path):
        """
        Write traces from Maxwell .h5 file to .npy for faster data retrieval

        :param: rec_path: str
            Path to raw Maxwell .h5 recording
        :param: npy_path: str
            Path to saved .npy
        """
        ...
        if "sig" in rec.keys():
            channels = [m[0] for m in rec["mapping"]]
            sig = rec['sig'][channels, :].astype(np.int16)
            np.save(npy_path, sig)
        else:
            assert NotImplementedError, "Cannot read from new Maxwell .h5 files"
...
class MultiRecordingDataset(Dataset):
    ...
    def __init__(self, 
                 ...
                 rec_paths, 
                 # WaveformDataset
                 thresh_amp, thresh_std, x_highest=None, use_positive_peaks=False, ms_before_after=None,
                 # Recording
        ...
        :param data_path: 
            List where each item is a path to dir containing:
                1. traces.npy: recording traces in shape of (num_chans, num_samples). Not scaled to uV
                2. sorted.npz: from spikesort_matlab4.py with SAVE_DL_DATA and SCALE_COMPILED_WAVEFORMS equal to True
        ...
        for path in rec_paths:
            path = Path(path)
            sorted = np.load(path / "sorted.npz", allow_pickle=True)
            fs = int(sorted["fs"] / 1000)  # Hz to kHz
            
            rec = Recording(rec_path=path, 
            ...
            if num_wfs_probs is None:
                rec_num_wfs_probs = np.load(path / "num_wfs_probs.npy")
    ...
    def __getitem__(self, idx):
        ...
        trace = self.recs[rec_i].get_sample()
...
class RecordingCrossVal:
    ...
    def __init__(self, 
                 ...
                 # Recording
                 sample_size, gain_to_uv, start=0, ms_before=3, ms_after=3, #, gain_to_uv=0.195 for neuropixels, use gain_to_uv=1 for mea-organoid since before 1/12/24, neither sorted.npz or traces.npy use gained values
                 # All
    ...
    def __getitem__(self, idx):
        ...
        return path_val_rec.name, train, val
    ...
    def summary(self, rec: None):
        """
        Print number of samples in each cross

        :param rec: str or int
            If not None, display summary of only rec
        """

        as_datasets = self.as_datasets
        self.as_datasets = True  # Makes it easier to find length of each dataset
        num_folds = len(self)
        if rec is None:
            for i, (rec, train, val) in enumerate(self):
                train = len(train)
                val = len(val)
                print(f"Fold: {i+1}/{num_folds} - Val Recording: {rec} - Train: {train} samples - Val: {val} samples - Train/Total: {train/(train+val) * 100:.2f}%")
        else:
            idx = self.name_to_idx(rec)
            rec, train, val = self[idx]
            train = len(train)
            val = len(val)
            print(
                f"Fold: {idx + 1}/{num_folds} - Val Recording: {rec} - Train: {train} samples - Val: {val} samples - Train/Total: {train / (train + val) * 100:.2f}%")
    ...
    def summarize(rec, train, val):
        ...
        print(f"Train num wfs probs:")
        for count, prob in enumerate(train.num_wfs_probs):
            print(f"{count}: {prob*100:.1f}%", end=", ")
        print(f"\nVal num wfs probs:")
        for count, prob in enumerate(val.num_wfs_probs):
            print(f"{count}: {prob*100:.1f}%", end=", ")
...
def is_dl_recording_folder(path: Path):
    """
    Check if path is a folder that represents a recording used for DL training/tuning
    Must have
        1. Name is a number
        2. Contains a data.raw.h5 file

    :param path:
    :return: bool
    """

    return path.name.isnumeric() and (path / "data.raw.h5").exists()
...
def main():
    ...
    cross_val = RecordingCrossVal(sample_size=200, front_buffer=40, end_buffer=40,
                                  num_wfs_probs=[1], isi_wf_min=5, isi_wf_max=None,
                                  thresh_amp=3, thresh_std=0.6,
                                  mmap_mode="r", as_datasets=False,
                                  samples_per_waveform=2000,
                                  batch_size=32, num_workers=0, shuffle=True
                                  )
    # Get distribution of input samples
    # for rec, train, val in cross_val:
    # rec, train, val = cross_val["5118"]
    # samples = [train[i][0].cpu().numpy() for i in range(1, len(train), 2)]
    # samples = samples
    ...
    # Plot samples from RecordingCrossVal
    for fold in cross_val:
        rec, train, val = fold
        print(f"Rec: {rec} - Train: {len(train)} - Val: {len(val)}")

        # as_datasets=False
        for inputs, num_wfs, wf_locs, wf_alphas in train:
    ...
    # Test WaveformDataset
    # REC_ROOT = Path(r"/data/MEAprojects/DLSpikeSorter")
    # for rec in REC_ROOT.iterdir():
    #     if not rec.name.isnumeric() or rec.name != "2950":
    #         continue
    #     wf = WaveformDataset(rec / "sorted.npz", thresh_amp=2.5, thresh_std=0.6, plotting=True)
    #     print(f"{rec.name}: {len(wf)}")
    #     wf.plot_waveforms()

    # Test MultiRecordingDataset
    # data = MultiRecordingDataset.load_single(r"C:\KosikLab\2950",
    #                                          samples_per_waveform=1, front_buffer=20, end_buffer=40,
    #                                          num_wfs_probs=[0, 0, 1], isi_wf_min=3, isi_wf_max=20,
    #                                          sample_size=200,
    #                                          thresh_amp=1.5, thresh_std=0.6)
    # for idx in range(len(data)):
    #     data.plot_sample(*data[idx])
...
class KilosortDLDataset(Dataset):
    def __init__(self, rec_path, dl_detector_path):
        ...
        with open(dl_detector_path / "init_dict.json", "r"):
            init_dict = json.load(dl_detector_path / "init_dict.json")
        print(init_dict)
    ...
    def rec_frame_to_dl_detector_frame(self, frame):
        pass
